    ...
    <div className="animate-in fade-in duration-500">
        ...
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div className="flex items-center">
              ...
              <span className="font-semibold text-[#004A8D] dark:text-blue-400">Listagem de Crianças</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="mb-6 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-end">
          ...
          <div className="flex-1 space-y-1">
            ...
            >
              <option value="">Todos</option>
              <option value="true">Dados Incompletos</option>
            </select>
          </div>
          <div>
            <button
              onClick={handleClearFilters}
              className="inline-flex h-10 items-center justify-center rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 transition-colors hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:focus:ring-slate-600"
            >
              Limpar
            </button>
          </div>
      ...
      <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
          ...
          <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {loading ? (
                ...
                children.map(child => (
                  <tr key={child.id} className="bg-white dark:bg-slate-900 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-6 py-4">
                      <div className="font-semibold text-[#004A8D] dark:text-blue-400">{child.nome}</div>
                      <div className="text-xs text-[#525F6A] dark:text-slate-500">{getIdade(child.data_nascimento)} anos</div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center rounded-md bg-slate-100 dark:bg-slate-800 px-2 py-1 text-xs font-medium text-[#525F6A] dark:text-slate-400 ring-1 ring-inset ring-slate-500/10 dark:ring-slate-500/20">
                        {child.bairro}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      {isComplete(child) ? (
                        <span className="text-xs font-semibold text-slate-600 dark:text-slate-400">Completas</span>
                      ) : (
                        <span className="text-xs font-semibold text-amber-600 dark:text-amber-500">Incompletas</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className={`h-2.5 w-2.5 rounded-full ${child.revisado_em ? "bg-emerald-500" : "bg-slate-300 dark:bg-slate-600"}`}></div>
                        <span className={`text-xs font-medium ${child.revisado_em ? "text-emerald-700 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        ...
                        className="inline-flex items-center justify-center rounded-md bg-[#004A8D] dark:bg-blue-600 px-3 py-1.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-[#00346f] dark:hover:bg-blue-700"
        ...
        {!loading && children.length > 0 && (
          <div className="flex items-center justify-between border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-6 py-3">
            ...
            <div className="flex flex-1 justify-between sm:justify-end gap-2">
              <button
                ...
                disabled={page === 1}
                className="relative inline-flex items-center rounded-md border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 focus:z-20 focus:outline-offset-0 disabled:opacity-50 disabled:cursor-not-allowed"
